...
def title():
    st.title("Stock Predictor Application")
    st.subheader("The Team")
    col1, col2 = st.columns(2)
    with col1:
        st.image("./Shatakshi.jpeg", width=200)
        st.text("Shatakshi Brijpuriya\nStudent ID: 21200268")
        
    with col2:
        st.image("./Yugandhara.jpeg",width=200)
        st.text("Yugandhara Sawant\nStudent ID: 21200235")
...
def plot_data():
    ...
    fig.layout.update(title_text='Time Series Data',xaxis_title='Years', yaxis_title='Stock Price', xaxis_rangeslider_visible = True)
...
def forecasting(ticker):
    ...
    start_train = st.date_input("Start date", date(2000, 1, 1), min_value=date(1980, 1, 1), max_value=date.today(), key='sd')
    end_train = st.date_input("End date", date.today(), min_value=date(1980, 1, 1), max_value=date.today(), key ='ed')
    ...
    # end_date = st.date_input("End date", date.today(), min_value=date.today(), max_value=date(2100, 1, 1), key ='ed')

    # st.write(data['Date'].dtypes)
    # last_date = datetime.fromtimestamp(data['Date'].iloc[-1])
    # diff = (date.today() - last_date).days
    # period = (end_date - date.today()).days + diff
    period = n_years * 365
    with st.spinner('Loading...'):
        m = Prophet()
        m.fit(df_train)
...
if __name__ == "__main__":
    ...
    companies = pd.read_csv("./nasdaq_screener_1636450584038.csv")
    stocks = companies[['Symbol']]
    ...
    with st.sidebar:
        ...
        if(Today<=Start):
            st.error('Error: End date must fall after start date.')
        menu = st.selectbox("Menu Options",('About us','Company Profile','Descriptive Data','Technical Indicators','Prediction')) 
